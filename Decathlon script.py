...
class TrackandField():
    ...
    def __init__(self):
        ...
        self.df = pd.DataFrame(columns=['Athlete Name', 'Year', 'Team', 'Score'])
    ...
    def selectDropdown(self):
        ...
        eventselect.select_by_visible_text("Decathlon")
        time.sleep(3)
    ...
    def dataframecreation(self):
        ...
        numbers_list = [x for x in time_list if x.isnumeric() or (x.replace('.', '', 1).isdigit() if x.count('.') == 1 else False)]

        data_tuples = list(zip(athlete_list[0:], year_list[0:], team_list[0:],
                               numbers_list[0:]))  # list of each players name and salary paired together
        temp_df = pd.DataFrame(data_tuples, columns=['Athlete Name', 'Year', 'Team',
                                                     'Score'])  # creates dataframe of each tuple in list
        self.df = pd.concat([self.df, temp_df])  # appends to master dataframe
        self.df.to_csv(r'C:\Users\marit\Downloads\Top_100_NCAA_Decathlon', index=False, header=True)
