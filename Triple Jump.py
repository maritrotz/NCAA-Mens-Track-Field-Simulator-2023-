...
class TrackandField():
    ...
    def __init__(self):
        ...
        self.df = pd.DataFrame(columns=['Athlete Name', 'Year', 'Team', 'Mark'])
    ...
    def selectDropdown(self):
        ...
        eventselect.select_by_visible_text("Triple Jump")
        time.sleep(3)
    ...
    def dataframecreation(self):
        ...
        mark_list = [x for x in time_list if (x.endswith('m')if x.count('.') == 1 else False)]



        data_tuples = list(zip(athlete_list[0:], year_list[0:], team_list[0:],
                               mark_list[0:]))  # list of each players name and salary paired together
        temp_df = pd.DataFrame(data_tuples, columns=['Athlete Name', 'Year', 'Team', 'Mark'])  # creates dataframe of each tuple in list
        self.df = pd.concat([self.df, temp_df])  # appends to master dataframe
        self.df.to_csv(r'C:\Users\marit\Downloads\Top_100_NCAA_Triplejump', index=False, header=True)
